...
function Navbar({logout,user, cart, addToCart, removeFromCart, clearCart, subTotal}) {
    ...
    const ref = useRef();
  return (
    <div className='flex flex-col py-2 md:flex-row md:justify-start bg-white justify-center items-center shadow-md sticky top-0 z-10'>
        ...
        <div  className="cart top-8 items-center absolute right-0 mx-5 flex">
            {dropdown && <div onMouseOver={()=>{setDropdown(true)}} onMouseLeave={()=>{setDropdown(false)}} className="profile-menu absolute right-12 shadow-lg bg-orange-300 top-10 px-5 rounded-md w-36">
        ...
        <div ref={ref} className={`w-72 h-[100vh] sidecart overflow-y-hidden absolute top-0 right-0 bg-orange-300 px-8 py-10 transition-transform ${Object.keys(cart).length === 0 ? 'translate-x-full' : 'translate-x-0'} `}>
            ...
            <div className='flex'>
            <Link href={'/checkout'}>
                <button className="flex items-center mx-auto  text-white bg-orange-500 border-0 py-2 px-2 focus:outline-none hover:bg-orange-700 rounded text-sm"><BsFillBagCheckFill className='m-1'/>Checkout</button>
            </Link>
            <button onClick={clearCart} className="flex items-center mx-auto  text-white bg-orange-500 border-0 py-2 px-2 focus:outline-none hover:bg-orange-700 rounded text-sm">Clear Cart</button>
            </div>
        </div>
    </div> 
  )
