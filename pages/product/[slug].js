...
function slug({buyNow,addToCart, products, variants }) {
  ...
  const checkServiceability = async() => {
    ...
    if(pinJson.includes(parseInt(pin))){
      ...
      setService(false);
      toast.error('Sorry ☹️ Your Pincode is not serviceable ', {
        position: "bottom-center",
        autoClose: 2000,
        hideProgressBar: false,
        closeOnClick: true,
  ...
  <div className="container px-5 py-16 mx-auto">
    <div className="lg:w-4/5 mx-auto flex flex-wrap">
      ...
      <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
        <h2 className="text-sm title-font text-gray-500 tracking-widest">HashCode</h2>
        <h1 className="text-gray-900 text-3xl title-font font-medium mb-1">{products.title} ({(products.size).toUpperCase()}/{(products.color).toUpperCase()})</h1>
        ...
        <div className="flex mt-6 items-center pb-5 border-b-2 border-gray-100 mb-5">
          <div className="flex">
            ...
            {Object.keys(variants).includes('red') && Object.keys(variants['red']).includes(size) && <button onClick={(e)=>{refreshVariant('red',size)}} className={`border-2  ml-1 bg-red-500 rounded-full w-6 h-6 focus:outline-none ${color == 'red'? 'border-black' : 'border-gray-300' }`}></button>}
            {Object.keys(variants).includes('blue') && Object.keys(variants['blue']).includes(size) && <button onClick={(e)=>{refreshVariant('blue',size)}} className={`border-2  ml-1 bg-blue-500 rounded-full w-6 h-6 focus:outline-none ${color == 'blue'? 'border-black' : 'border-gray-300' }`}></button>}
            {Object.keys(variants).includes('green') && Object.keys(variants['green']).includes(size) && <button onClick={(e)=>{refreshVariant('green',size)}} className={`border-2  ml-1 bg-green-500 rounded-full w-6 h-6 focus:outline-none ${color == 'green'? 'border-black' : 'border-gray-300' }`}></button>}
            {Object.keys(variants).includes('purple') && Object.keys(variants['purple']).includes(size) && <button onClick={(e)=>{refreshVariant('purple',size)}} className={`border-2  ml-1 bg-purple-500 rounded-full w-6 h-6 focus:outline-none ${color == 'purple'? 'border-black' : 'border-gray-300' }`}></button>}
            {Object.keys(variants).includes('yellow') && Object.keys(variants['yellow']).includes(size) && <button onClick={(e)=>{refreshVariant('yellow',size)}} className={`border-2  ml-1 bg-yellow-500 rounded-full w-6 h-6 focus:outline-none ${color == 'yellow'? 'border-black' : 'border-gray-300' }`}></button>}
            {Object.keys(variants).includes('black') && Object.keys(variants['black']).includes(size) && <button onClick={(e)=>{refreshVariant('black',size)}} className={`border-2  ml-1 bg-black rounded-full w-6 h-6 focus:outline-none ${color == 'black'? 'border-black' : 'border-gray-300' }`}></button>}
          </div>
